...
Arguments: `<question about how something works or what it should do>`. Use the values supplied in the user’s request.

Answer this question about the codebase/product: **the request arguments**
...
- **Facts come from the repo, not memory.** Establish current behavior by reading the relevant code directly; when the question needs a genuinely broad survey (many call sites, several subsystems), fan out independent read-only agents when available; otherwise inspect directly in parallel instead. Every claim in the answer is anchored to file:line either way.
