...
2. **Regressions** — did behavior change where the plan said behavior-preserving? Diff semantics, not syntax: lock/visibility semantics (`Clock`-derived, query-layer enforced), auth/role/allowlist gates, transaction ordering, settlement purity (would a full recompute reproduce this state?), provider tables written only by sync jobs (ADR-0046), job idempotency, contract drift (does `openapi/` regenerate clean?).
3. **Standards** — violations of `docs/harness/engineering.md`, `docs/architecture.md`, or `docs/mvp-spec.md` introduced by the diff. Raw `Date.now()`/SQL `now()` in domain logic is always a blocker.
