...
- **Cancelled game:** pick resolves as a push — the member survives and the team is **not** consumed (available for future use). A game the provider moves to another week is not a modelled event in either NFL mode (ADR-0019); an admin corrects it by setting the game's status to `cancelled` (a hand database edit since ADR-0046 — `docs/runbooks/jobs.md`), which lands here.
