...
### Temporary screenshots for PR review

Review screenshots must stay out of Git, including `docs/evidence/`; retain scripts and short verification notes only. For a focused capture, write to `test-results/review-screenshots/<feature>/` (ignored), and let its capture spec opt in via `REVIEW_CAPTURE=1` alongside any feature-specific flag. Do not enable unrelated full-surface captures. Run locally, inspect the affected states at 390px and 1024px, and check the applicable themes before sharing.

Add the `visual-review` label to the PR before pushing the capture changes. CI runs the ordinary test suite, then the opt-in simulated capture specs serially, and uploads only `test-results/review-screenshots/` as `review-screenshots-pr-<number>` with a **seven-day retention period**. Applying the label also triggers CI. A labeled PR without a capture producing files fails the artifact step instead of claiming screenshots exist. Keep the label while the PR needs refreshed evidence on new commits; remove it when captures are no longer needed.

After the run succeeds, inspect the generated images and copy the artifact download link from the Actions run into the PR description with the commit and expiry date. Verify the link and remove local output after upload; never force-add ignored screenshots as a workaround for upload trouble. If artifact upload is unavailable, share local previews with the owner and state the limitation in the PR. Expired artifacts can be regenerated by rerunning CI while the label remains. Avoid embedding repository image URLs or retaining duplicate PR attachments that bypass expiry.
