...
6. [NFL Phases & Scheduling](#6-nfl-phases--scheduling)
7. [Picks](#7-picks)
...
PicksLeagues is a sports prediction app. The MVP is an NFL Pick'Em game mode. Users create or join private leagues, make weekly game picks (straight-up or against the spread), and compete on a season-long leaderboard.
...
3. Each NFL phase (week), the user selects a configured number of game winners before the pick lock deadline
...
## 6. NFL Phases & Scheduling

### 6.1 Concepts

The NFL season is divided into **phases** (e.g., "Week 1", "Wild Card", "Divisional Round"). Each phase has:

- A label and sequence number defining its order in the season
- A start date and end date
- A calculated pick lock time

> **Note on naming**: We use "phase" as the generic term for a time period in a season. For the NFL, each phase corresponds to a week. This naming convention supports future game modes where phases may represent tournament rounds (March Madness), tournament days (golf/tennis), or playoff rounds (NBA).
...
When a user views their league, the system determines the "current phase":

- If a phase is currently active (today falls between its start and end date, within the league's configured phase range), that phase is shown.
- If no phase is currently active, the **next upcoming phase** is shown.
...
- Pre-season and off-season phases are excluded.
- **Pro Bowl** phases are excluded.
...
```
Total Points = Wins + (Pushes x 0.5)
```
...
The application syncs NFL data from ESPN's public API (no API key required). This populates seasons, phases, teams, game schedules, odds, and live scores.
...
1. Seasons -> 2. Phases -> 3. Teams -> 4. Games -> 5. Games + Odds -> 6. Games + Live Scores -> 7. Standings Calculation
...
- Pick lock times are calculated during the phases sync based on the rules in [Section 6.2](#62-pick-lock-time).
...
The league list page shows all of a user's leagues as cards displaying: league name, image, pick type label, and picks-per-phase count.
...
**My Picks** — The user's picks for the current (or selected) phase. Shows:
...
- Phase navigation (previous/next) for historical browsing

**League Picks** — All members' picks for a phase. Before lock time: shows a message that picks will be visible after the deadline. After lock time: collapsible cards per member showing their picks, record, and points.
...
2. If it's the current phase and before lock time:
...
| Rule                    | Value                           |
| ----------------------- | ------------------------------- |
| Picks per phase range   | 1–16                            |
| Default picks per phase | 5                               |
...
| **Mobile App**     | Native mobile experience. Added if demand materializes.                 |
...
- **Sports leagues** — `sportsLeagues` table supports NFL, NCAA, PGA, ATP/WTA, NBA
- Sport-specific logic will live in isolated modules (e.g., `lib/nfl/`, `lib/march-madness/`)

---

_Adapted from legacy business spec — 2026-04-08_
