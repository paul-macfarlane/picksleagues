...
- [x] **ELM-11** — Eliminate a member as soon as their loss is certain, rather than making them wait for the whole week to grade (owner, 2026-08-08). Today a Survivor week grades as a unit (ADR-0025): a member who backed a loser stays `alive` on the board until every game in the week is final, which reads as a missed elimination. The unit rule exists for two week-total questions — whether *everyone* alive busted (revival), and whether a member missed their pick, which is not knowable while any game is still unstarted. But once **one** member's pick is confirmed correct, revival provably cannot fire that week, so every confirmed-losing pick in it can be settled immediately. Scope is the partial-grading path and what it does to the prefix invariant settlement relies on: a partly-graded week must still be recomputable (arch D10) and must not let the replay treat it as complete. Ref: spec §Game Mode 2 — Core Rules. **Delivered 2026-08-08 (same session it was filed), recorded as ADR-0028:** `settleSurvivorWeekProvisionally` sits beside the untouched complete-week grader; only `survivor_state` is written early — no result rows, because `teamConsumed` feeds the whole-season `released` ledger; a missed pick is never provisional; the replay still stops on the partly-graded week, so the prefix invariant stands. _(deps: ELM-4)_
