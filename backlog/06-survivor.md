...
- [~] **ELM-8** — Survivor board and pick-sheet finish, from the owner's first real pass over ELM-4. Team logos on the board beside the abbreviations (`TeamLogo` exists and the endpoint already serves `logoLightUrl`/`logoDarkUrl` — the board simply never rendered them). Settled picks carry their outcome on the **pick sheet**, with the fill/border/badge treatment `pickem-game-row.tsx` already has — the week-picks DTO has no `outcome` field at all today, so this is a schema, serializer and contract change before it is a UI one. A locked week shows the member their own game rather than a slate of disabled buttons: the existing "keep closed games on screen" rationale is about someone who can still act, and once their pick is frozen they cannot. And the sheet says when a week has not been **graded** yet — the board already reports this (`Nothing has settled yet`), the sheet does not, which is the whole of why a cancelled game reads as a bug. Ref: spec §Game Mode 2; §Screens. _(deps: ELM-4)_
- [~] **ELM-9** — End a Survivor season once one member is left, instead of making a decided winner keep picking against nobody (owner, 2026-08-08). Spec §End of League currently concludes a league only when the last week of its resolved range settles, so this **deviates from the locked spec and needs an ADR plus the matching spec amendment before code**. It is safe today for a reason worth recording in that ADR: with revival mandatory a sole survivor cannot be eliminated — busting makes them the entire alive set, which the everyone-out rule revives — so the result is already decided and this only tells the truth earlier. The season's decided state needs **one home** shared by the board, the pick endpoint and the dashboard glance, not a third private copy of the question; folding the existing `isSeasonConcluded` into it is in scope. _(deps: ELM-4)_
- [ ] **ELM-10** — Make the everyone-out revival rule configurable (owner, 2026-08-08): reviving every member who busts in the same week is one of several real survivor-pool answers, not the standard one — pools also end there and split, void and re-run the week, or simply continue with nobody. Additive settings change with a Zod `.default()` so stored rows still parse, but it deviates from spec §Game Mode 2, which states revival unconditionally, so it needs an ADR first. **Blocked on a second decision it must make:** revival being mandatory is exactly what makes ELM-9 safe, so with it disabled a lone survivor can bust to zero and the pool has to say who wins when nobody is standing — and ELM-9's early-end rule has to become conditional on the setting. _(deps: ELM-9)_
