...
- [ ] **ELM-10** — Make the everyone-out revival rule configurable (owner, 2026-08-08): reviving every member who busts in the same week is one of several real survivor-pool answers, not the standard one — pools also end there and split, void and re-run the week, or simply continue with nobody. Additive settings change with a Zod `.default()` so stored rows still parse, but it deviates from spec §Game Mode 2, which states revival unconditionally, so it needs an ADR first. **Carries a second decision:** with revival disabled the final group of **two or more** alive members can all bust in the same week, taking a league from N to zero without ever passing through a single survivor — so the pool has to say who wins when nobody is standing. That is this ticket's own question, arising from the everyone-out week; it does **not** constrain ELM-9, whose early end removes the later pick that would be needed to unsettle it. _(deps: none)_
