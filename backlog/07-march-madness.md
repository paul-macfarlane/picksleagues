...
Trimmed before build by SWP-6 (owner, 2026-08-09; ADR-0034): scoring is
Standard Doubling only (no Custom model), and the pre-deadline seed-correction
wipe-and-resubmit flow is an admin-by-hand procedure. MM-1/3/4/6 below reflect
the trim; vacated-slot auto-advance stays — it is grading correctness.
...
- [x] **MM-1** — `MarchMadnessSettings` Zod schema (max brackets 1–10) + settings form. _(deps: LG-2)_ (landed with LG-2, not as a dedicated MM task; the custom scoring model it originally carried was cut by SWP-6/ADR-0034)
...
- [ ] **MM-4** — `scoreBracket` pure function + table-driven tests: standard doubling (the only model — ADR-0034), correct-regardless-of-path, vacated/cancelled as push with slot auto-advance neutrality, absolute-difference tiebreaker. _(deps: FND-7)_
- [ ] **MM-5** — Bracket builder UI (mobile-first 63-pick flow) + bracket views. _(deps: MM-3)_
- [ ] **MM-6** — Settlement integration + pool leaderboard (one row per bracket). Seed corrections are an admin-by-hand procedure, not a flow (ADR-0034). _(deps: MM-4, PKM-4)_
