...
- [ ] **ALN-4** — Owner triage of the ALN-3 report (`docs/sweeps/ALN-3-spec-vs-app.md` — 10 product questions P1–P10, 5 extensions E1–E5, 9 doc-stale rows S1–S9): each `differs` and `not implemented` row gets a verdict — keep the app (doc moves), fix the app (task appended to the owning epic), or drop the rule (doc deletes it). Human-only; the agent's job is to present the rows and record the answers. _(deps: ALN-3)_ _(ready-for-human)_
