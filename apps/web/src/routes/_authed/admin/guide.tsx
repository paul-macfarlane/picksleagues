...
export const Route = createFileRoute("/_authed/admin/guide")({
  component: AdminGuide,
});

// Operator-shaped consolidation of docs/runbooks/jobs.md and
  ...
  return (
    <div className="max-w-2xl">
      <StaticProse>
        <section>
          <p>
            This page is the manual for the admin side of the app: what each tab does, how the data
            gets here, and what to do when something looks wrong. Admin is an app-wide operator
            role, separate from being a league commissioner — it is granted only by a direct
            database update, checked on the server for every request, and invisible to everyone
            else. Everything below operates on live data.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            ...
            <strong>wrong at the source</strong>, you correct it with an override — never by waiting
            for the provider to fix itself.
          </p>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>
              <strong>Sync schedule</strong> — seasons, weeks, games, and kickoff times. Runs twice
              a day; in the offseason it also creates the upcoming season on its own.
            </li>
            <li>
              <strong>Sync odds</strong> — point spreads for the current week and the next one. Runs
              three times a day in season.
            </li>
            <li>
              <strong>Sync scores</strong> — live and final scores, every 15 minutes. As a game goes
              final this job also grades its picks and updates standings, so results normally appear
              within minutes without anyone doing anything.
            </li>
            <li>
              <strong>Sync stats</strong> — team records, season stats, and the matchup sheet&apos;s
              injury/context data. Runs twice a day; the sheet shows its own &quot;as of&quot;
              stamp.
            </li>
          </ul>
          <p>
            A fifth job, the <strong>settlement sweep</strong>, has no button: it re-derives every
            league&apos;s results and standings from stored data twice a day as a safety net, so any
            correction or missed tick is reconciled by the next sweep at the latest.
          </p>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>
              <strong>Seasons</strong> — read-only view of what the schedule sync ingested: weeks
              and their date windows, linking into each week&apos;s games.
            </li>
            <li>
            ...
            <li>
              <strong>Teams</strong> — team identity (name, abbreviation, location, logos) with its
              own override editor. Identity shows up everywhere in the app, so this is where a
              provider&apos;s wrong logo or renamed team gets fixed once.
            </li>
            <li>
              <strong>Stats</strong> — what the stats sync wrote: per-team season stats and records,
              and each game&apos;s matchup context, with as-of stamps and override editors.
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul>
            <li>
              <strong>Game overrides</strong> (kickoff, status, scores, spread) feed real outcomes:
              correcting a final score or status re-grades the affected picks and standings the next
              time the game settles — at the latest, the next settlement sweep.
            </li>
            <li>
        ...
        <section>
          <h2>Audit</h2>
          <p>
            Two things live here. The <strong>data integrity</strong> card flags games whose kickoff
            is still ahead while their status or score already gives the outcome away — members can
            still pick those, so correct the kickoff or the result when one appears. Below it, the{" "}
            <strong>audit log</strong> is the permanent trail of every override and admin action.
          </p>
        </section>
        ...
        <section>
          ...
          <ul>
            ...
            <li>
              ...
              cycle — corrections flow into standings when settlement runs, and the sweep reconciles
              everything twice a day regardless.
            </li>
            <li>
        ...
        {simEnabled && (
          <section>
            ...
            <p>
              This environment has the season simulator: a movable clock plus swappable season data,
              driven from the <Link to="/sim">Simulator</Link> section. Two rules explain nearly
              every surprise. First, the clock and the scenario are{" "}
              ...
              simulator feeds the same jobs pipeline as real data, so every move is two steps: move
              the clock (or load a scenario), then run the sync.
            </p>
            <ul>
