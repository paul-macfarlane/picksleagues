...
- Against the current `[~]` in-flight set, mark which runnable tasks are **parallel-safe** — no plausible file-surface overlap with in-flight work (same package, `openapi/` regeneration, same epic file). Those are the candidates for a second session via `/worktree`; overlapping ones get flagged sequential.
