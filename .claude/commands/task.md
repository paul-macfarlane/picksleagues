...
**Rule of three.** "Follow the pattern of X exactly" is the right dispatch instruction for consistency — and it faithfully propagates boilerplate. Every implementer dispatch must carry the counterweight: *if the spec would have you create a third copy of anything (a constant, a guard block, a response descriptor, a test fixture), extract it to a shared home first and report the extraction.* When you as orchestrator already know a pattern exists twice, extract before dispatching, not after.
