...
- **Value sets are const objects** + `as const` with a derived literal-union type (game status, league mode, pick outcome, member role, push/tie resolution). The TS `enum` keyword is banned. Non-test code never compares raw literals of a set; tests MAY assert raw literals — they pin the contract so a constant-value edit fails loudly.
