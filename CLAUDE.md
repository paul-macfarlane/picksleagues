...
PicksLeagues is a sports prediction app. The MVP is an NFL Pick'Em game mode where users create/join private leagues, make weekly picks (straight-up or against the spread), and compete on a season-long leaderboard. Built with Next.js 16 (App Router), TypeScript, Drizzle ORM, Neon PostgreSQL, Better Auth, shadcn/ui, and Tailwind CSS v4. Future game modes include March Madness pools, Golf/Tennis major pools, and NBA playoff pools.
...
5. **Update work/Backlog.md** as you go — mark tasks `[~]` when starting, `[x]` when done
...
- **Sport-agnostic naming** in schema: phases (not weeks), events (not games)
- **Sport-specific logic** in namespaced directories: `lib/nfl/`, `lib/espn/nfl/`, `lib/sync/nfl/`
- **Mobile-first UI** — phone is the primary device
